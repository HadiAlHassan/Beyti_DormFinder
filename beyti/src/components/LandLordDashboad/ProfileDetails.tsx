...
type Landlord = {
  ...
  dob: string;
  gender: string;
  phone_number: string; // 🔥 fixed to string
  email: string;
  picture?: {
    data: string;
    contentType: string;
...
export default function ProfileDetails() {
  ...
  useEffect(() => {
    const { userId, role, token } = getCookie(); // 🛠 pull token too
    if (!userId || role !== "landlord" || !token) {
      console.warn("Missing or invalid role/userId/token in cookie.");
      return;
    }
    ...
    const fetchLandlord = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/landlord/${userId}`,
          {
            method: "GET",
            headers: {
        ...
        <div className="container mx-auto">
          ...
          <div className="mb-10">
            <p className="text-primary font-bold pb-10 pt-8 text-2xl text-center">
              Welcome {landlord.gender === "male" ? "Mr." : "Ms."}{" "}
              {landlord.first_name}
            </p>
